...
const getPieczarsonMatchInfo = `${EUW_API_BASE_MATCH_URL}EUW1_4558360996`;


/* GET users listing. */
router.get('/by-summoner/:id', async function (req, res, next) {
  try {
    const query = req.params.id;
    const response = await got(`${EUW_API_BASE_MATCH_URL}${GET_SUMMONER_MATCHES}${query}/ids?count=${20}`, {
      headers: {
...
router.get('/info/:id', async function (req, res, next) {
  try {
    const query = req.params.id;
    const response = await got(`${EUW_API_BASE_MATCH_URL}${query}`, {
      headers: {
        'X-Riot-Token': 'RGAPI-07861a03-575a-4f9e-9c52-f7b4e48e88fe'
      }
    }).json();
